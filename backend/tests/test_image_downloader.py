...
async def test_skips_low_resolution_then_uses_next():
    ...
    respx.get(small_url).mock(
        return_value=httpx.Response(200, content=_make_jpeg_bytes(100, 100))
    )
    respx.get(big_url).mock(
        return_value=httpx.Response(200, content=_make_jpeg_bytes(800, 600))
    )
...
async def test_front_image_is_preferred():
    ...
    respx.get(front).mock(
        return_value=httpx.Response(200, content=_make_jpeg_bytes(800, 800))
    )
    respx.get(side).mock(
        return_value=httpx.Response(200, content=_make_jpeg_bytes(800, 800))
    )
